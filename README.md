...
`NoisyLR` input, a `Bicubic` upsampling baseline, and the restored output
generated by the trained `SRDnCNN` model.

| Method | Description |
|---|---|
| **NoisyLR** | Original degraded low-resolution semiconductor inspection image |
| **Bicubic** | Conventional 2× bicubic upsampling baseline |
| **Exp2 SRDnCNN** | Final selected trained SRDnCNN restoration model |

![Phase 9.5 Final Test-Set Qualitative Comparison](results/final_test/phase9_5_final_test_contact_sheet.png)

**Figure 20.1:** Representative final test-set comparison showing the degraded
NoisyLR input, Bicubic upsampling baseline, and Exp2 SRDnCNN restoration output.

> **Note:** The final test set contains degraded images without corresponding
> ground-truth images. Therefore, this phase provides qualitative visual
> inspection only; PSNR, SSIM, and LPIPS are not reported for these samples.
